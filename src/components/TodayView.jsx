...
import { computeStreak, dayStats, getBacklog, groupBySection } from '../lib/habits.js'
...
export default function TodayView() {
  ...
  async function onExtra(templateId) {
    await logHabitExtra(templateId, day)
    notifyRefresh('habits')
    load()
  }
  ...
  return (
    <section className="section today-section">
      ...
      <label className="flag handwriting-flag">
        ...
        <span>Scritto a mano oggi (se no → stare al muro)</span>
      </label>

      {loading && <p className="muted">Caricamento...</p>}
      {!loading && logs.length === 0 && <p className="empty-hint">Nessun check per questo giorno.</p>}
            ...
            {items.map((item) => (
              <li key={item.log.id}>
                <div className="check-row">
                  <label className="check-row-inner">
                  ...
                  {item.allow_multi && (
                    <button type="button" className="btn-icon" title="Logga di nuovo" onClick={() => onExtra(item.id)}>+</button>
                  )}
                </div>
              </li>
            ))}
      ...
      {backlog.length > 0 && (
        <div className="backlog-block">
          <button type="button" className="collapse-head" onClick={() => setBacklogOpen(!backlogOpen)}>
            <h3 className="sub-head">Settimana / mese</h3>
            ...
            <ul className="backlog-list">
              {backlog.map((b) => (
                <li key={b.template.id} className="backlog-item">
                  <span>{b.template.title} ({b.done}/{b.quota} {b.period})</span>
                  <button type="button" className="btn-chip" onClick={async () => { await markHabitOnDay(b.template.id, day); notifyRefresh('habits'); load() }}>Segna</button>
                </li>
              ))}
            </ul>
          )}
